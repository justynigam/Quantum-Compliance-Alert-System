        ...
        for transaction in transactions_created:
            if transaction.status in [Transaction.Status.HIGH_RISK, Transaction.Status.BLOCKED]:
                try:
                    ...
                    explanation_data = XAIService.generate_explanation(features)
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"Could not create explanation for {transaction.transaction_id_str}: {str(e)}")
                    )

        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(transactions_created)} transactions and {explanations_created} explanations.'))
